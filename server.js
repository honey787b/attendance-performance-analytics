...
app.get("/api/timetable", async (req, res) => {
  try {
    ...
    `;
    const params = [];
    if (branch) { sql += " AND t.branch = ?"; params.push(branch); }
...
app.post("/api/timetable", async (req, res) => {
  try {
    const { day_name, start_time, end_time, subject_id, faculty_id, branch, year, section, room } = req.body;
    const [result] = await pool.query(
      `INSERT INTO timetable (day_name, start_time, end_time, subject_id, faculty_id, branch, year, section, room)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
app.post("/api/attendance/sessions", async (req, res) => {
  try {
    const { session_date, branch, year, section, subject_id, faculty_id } = req.body;
    const [result] = await pool.query(
      `INSERT INTO attendance_sessions (session_date, branch, year, section, subject_id, faculty_id, status)
       VALUES (?, ?, ?, ?, ?, ?, 'Active')`,
      [session_date, branch, year, section, subject_id || null, faculty_id || null]
    );
    res.status(201).json({ id: result.insertId });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get("/api/attendance/rules", async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM attendance_rules ORDER BY id DESC LIMIT 1");
    res.json(rows[0] || {});
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/attendance/rules", async (req, res) => {
  try {
    const { excellent, good, warning, critical, minimumAttendance, lateLimit } = req.body;
    const [existing] = await pool.query("SELECT id FROM attendance_rules ORDER BY id DESC LIMIT 1");

    if (existing.length) {
      await pool.query(
        `UPDATE attendance_rules SET excellent_threshold=?, good_threshold=?, warning_threshold=?,
         critical_threshold=?, minimum_attendance=?, late_limit=? WHERE id=?`,
        [excellent, good, warning, critical, minimumAttendance, lateLimit, existing[0].id]
      );
    } else {
      await pool.query(
        `INSERT INTO attendance_rules
         (excellent_threshold, good_threshold, warning_threshold, critical_threshold, minimum_attendance, late_limit)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [excellent, good, warning, critical, minimumAttendance, lateLimit]
      );
    }
    res.json({ ok: true });
  } catch (err) {
...
app.get("/api/marks", async (req, res) => {
  try {
    ...
    if (subject_id) { sql += " AND im.subject_id = ?"; params.push(subject_id); }
    if (semester) { sql += " AND im.semester = ?"; params.push(semester); }
...
app.post("/api/marks", async (req, res) => {
  try {
    const { student_id, subject_id, semester, quiz, assignment, mid1, mid2, semester_exam, max_marks } = req.body;
    await pool.query(
      `INSERT INTO internal_marks (student_id, subject_id, semester, quiz, assignment, mid1, mid2, semester_exam, max_marks)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE quiz=VALUES(quiz), assignment=VALUES(assignment),
         mid1=VALUES(mid1), mid2=VALUES(mid2), semester_exam=VALUES(semester_exam), max_marks=VALUES(max_marks)`,
      [student_id, subject_id, semester || 1, quiz || 0, assignment || 0, mid1 || 0, mid2 || 0, semester_exam || 0, max_marks || 100]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    const [rows] = await pool.query(`
      SELECT s.id, s.roll_no, s.name, s.branch, s.attendance_percent, s.gpa,
        ...
        END AS risk_type
      FROM students s, (SELECT * FROM attendance_rules ORDER BY id DESC LIMIT 1) r
...
app.post("/api/holidays", async (req, res) => {
  try {
    const { holiday_date, name, type, icon, description } = req.body;
    const [result] = await pool.query(
...
app.get("/api/reports", async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM reports ORDER BY generated_date DESC");
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/reports", async (req, res) => {
  try {
    const { report_name, type, period, generated_by } = req.body;
    const [result] = await pool.query(
      `INSERT INTO reports (report_name, type, period, generated_by, status) VALUES (?, ?, ?, ?, 'Ready')`,
...
app.get("/api/settings", async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM institution_settings ORDER BY id DESC LIMIT 1");
    res.json(rows[0] || {});
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/settings", async (req, res) => {
  try {
    const {
      ...
      attendance_alerts_enabled, attendance_notifications_enabled,
      performance_alerts_enabled, report_notifications_enabled,
    } = req.body;

    const [existing] = await pool.query("SELECT id FROM institution_settings ORDER BY id DESC LIMIT 1");

    if (existing.length) {
      await pool.query(
        `UPDATE institution_settings SET academic_year=?, current_semester=?, default_department=?,
         attendance_alerts_enabled=?, attendance_notifications_enabled=?,
         performance_alerts_enabled=?, report_notifications_enabled=? WHERE id=?`,
        [academic_year, current_semester, default_department,
         attendance_alerts_enabled, attendance_notifications_enabled,
         performance_alerts_enabled, report_notifications_enabled, existing[0].id]
      );
    } else {
      await pool.query(
        `INSERT INTO institution_settings
         (academic_year, current_semester, default_department, attendance_alerts_enabled,
          attendance_notifications_enabled, performance_alerts_enabled, report_notifications_enabled)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [academic_year, current_semester, default_department, attendance_alerts_enabled,
         attendance_notifications_enabled, performance_alerts_enabled, report_notifications_enabled]
      );
...
app.get("/api/dashboard/stats", async (req, res) => {
  try {
    ...
    const [[{ avgAttendance }]] = await pool.query("SELECT ROUND(AVG(attendance_percent),2) AS avgAttendance FROM students");
    const [[{ atRiskCount }]] = await pool.query(`
      SELECT COUNT(*) AS atRiskCount FROM students s, (SELECT * FROM attendance_rules ORDER BY id DESC LIMIT 1) r
      WHERE s.attendance_percent < r.warning_threshold OR s.gpa < 2.5
    `);
    res.json({ totalStudents, totalFaculty, avgAttendance, atRiskCount });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
